...
class ManageUsedCarListingEntity{
    ...
    public function getfavcar(){
        $stmt = $this->conn->prepare("SELECT c.carID.carName, u.username, c.price, c.favourites, c.views FROM carlisting c INNER JOIN useraccount u ON c.agent = u.id WHERE u.profileId = 2;");
        $stmt->execute();
